...
msxserver
alias m='msx(){ [[ -z "$2" ]] && mocp --info | grep -E --color=never "^(State|File|Title|TotalTime|TimeLeft)" && echo "*";if [ -z "$1" ] || [ "$1" = "--" ];then [ -d $SH_MSX/$2 ] && ls -1 "$SH_MSX/$2"; else [ -d $SH_MSX/$1 ] && mocp -c && mocp --append $SH_MSX/$1 && mocp --play; fi; };msx'
