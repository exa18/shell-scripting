...
alias U='sudo echo -n "#";sudo apt update >/dev/null 2>&1;echo -ne "\r";apt upgrade -s 2>/dev/null | grep -E "(^Inst|upgraded)"|awk '"'"'{if (NR>2) {print $2" .. "$3" to "$4")"} else {print}}'"'"';'
# used with ubuntu
...
[[ -n $(command -v timeshift) ]] && alias ts='fn_ts(){ if [[ "${1::2}" = "--" ]]; then x=${1#--*}; [[ -z "$x" ]] && x=0; for ((i=0 ; i<=$x; i++)); do fn_timeshift 0; done; else if [[ -n "$1" ]]; then fn_timeshift $1; sleep 1; fi; sudo timeshift --list; fi;}; fn_timeshift(){ tls=$(sudo timeshift --list | awk -v s=$1 '"'"'$1==s {print $3}'"'"'); sudo timeshift --delete --snapshot $tls;};fn_ts'
